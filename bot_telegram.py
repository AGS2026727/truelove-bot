...
def construir_system_prompt(conselheiro: str, lang: str, nome: str, genero: str, pronomes: str) -> str:
    prompt = PROMPTS.get(conselheiro, {}).get(lang, "")
    
    # Se por algum motivo o arquivo TXT falhar ou sumir, usa um fallback básico
    if not prompt:
        prompt = f"You are {conselheiro}, a specialized relationship advisor."
...
async def conselheiro_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if "Luna" in texto:
        ...
        saudacao_pt = f"Nossa, que bom que você veio, {nome}. Sou a Luna. Pode chegar do jeito que estiver, bagunçada, confusa, com raiva ou sem saber nem por onde começar, aqui comigo você está seguro, não tem julgamento nenhum. Me conta o que aconteceu de verdade."
        saudacao_en = f"Wow, so good to see you here, {nome}. I'm Luna. Come as you are, messy, confused, angry or not even knowing where to start. With me you are safe, no judgment here. Tell me what really happened."
    elif "Kai" in texto:
        context.user_data["conselheiro"] = "Kai"
        saudacao_pt = f"E aí, {nome}. Sou o Kai. Sem julgamento nenhum aqui, irmão. Abre o jogo e fala o que você realmente está sentindo. O que está acontecendo?"
        saudacao_en = f"Hey, {nome}. I'm Kai. No judgment here, brother. Open up and tell me what you are really feeling. What's going on?"
    elif "Maya" in texto:
        context.user_data["conselheiro"] = "Maya"
        saudacao_pt = f"Caramba, {nome}... que bom te ver por aqui. Sou a Maya. Vai, desabafa e não fica com vergonha. Aqui comigo você está seguro e sem filtros. O que está passando pela sua cabeça agora?"
        saudacao_en = f"Damn, {nome}... so good to see you here. I'm Maya. Go on, don't be shy. With me you are safe and without filters. What's going through your head right now?"
    else:
        context.user_data["conselheiro"] = "Theo"
        saudacao_pt = f"E aí {nome}, sou o Theo. Aqui o papo é reto, sem enrolação e sem julgamento. Solta o que está na sua cabeça."
        saudacao_en = f"Hey {nome}, I'm Theo. Straight talk here, no nonsense and no judgment. What's on your mind?"
...
async def chat_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        if lang == "pt":
            resposta = "Desculpe, tive um problema técnico aqui. Tenta de novo em instantes."
        else:
...
async def cancelar(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    lang = context.user_data.get("lang", "pt")
    msg = "Até logo! Quando quiser, é só mandar um /start." if lang == "pt" else "See you! Whenever you're ready, just send /start."
